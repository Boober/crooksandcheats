...
{
	mongoDB.collection("abilities").insertOne(
		data,
		function(err,status)
		{
			if (err) doError(err);
			...
			callback(success);
			console.log("Insert into Abilities successful.");
		});
}
...
{
	mongoDB.collection("abilities").find(query).toArray(function(err,docs)
	{
		if (err) doError(err);
...
{
	mongoDB.collection("abilities").find({}).toArray(function(err,docs)
	{
		if (err) doError(err);
		callback(docs);
	});
...
{
	mongoDB.collection("abilities").updateMany(
		filter,
		update,
		{upsert:true},
...
{
	mongoDB.collection("abilities").deleteMany(
		query,
		function(err,status)
		{
			if (err) throw err;
