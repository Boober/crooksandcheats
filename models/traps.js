...
{
	mongoDB.collection("traps").insertOne(
		data,
		function(err,status)
		{
			if (err) doError(err);
			...
			callback(success);
			console.log("Insert into Traps successful.");
		});
}
...
{
	mongoDB.collection("traps").find(query).toArray(function(err,docs)
	{
		if (err) doError(err);
...
{
	mongoDB.collection("traps").find({}).toArray(function(err,docs)
	{
		if (err) doError(err);
		callback(docs);
	});
...
{
	mongoDB.collection("traps").updateMany(
		filter,
		update,
		{upsert:true},
...
{
	mongoDB.collection("traps").deleteMany(
		query,
		function(err,status)
		{
			if (err) throw err;
